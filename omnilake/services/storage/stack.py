...
class StorageManagerStack(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str,
        ...
        """
        Storage manager stack for OmniLake.
        ...
        self.storage_provisioner = EventBusSubscriptionFunction(
            base_image=self.app_base_image,
            construct_id='storage_provisioner',
            description='Provisions storage resources',
            entry=self.runtime_path,
            event_type='create_archive',
            index='provisioner.py',
            handler='handler',
            function_name=resource_namer('storage-provisioner', scope=self),
            memory_size=512,
            resource_access_requests=[
                ResourceAccessRequest(
        ...
        self.entry_index_event = EventBusSubscriptionFunction(
            base_image=self.app_base_image,
            construct_id='entry_index_event',
            description='Indexes an entry into a vector store for the requested archive.',
            entry=self.runtime_path,
            event_type='index_entry',
            index='vectorize.py',
            handler='handler',
            function_name=resource_namer('entry-indexer', scope=self),
            memory_size=512,
            managed_policies=[
